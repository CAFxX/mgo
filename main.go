...
func main() {
	...
	mgoGzip := 0
	if mg := os.Getenv("MGO_GZIP"); mg != "" {
		level, err := strconv.Atoi(mg)
		if err != nil || level < 0 || level > 9 {
			fmt.Printf("MGO_GZIP=%q is not valid", mg)
			os.Exit(1)
		}
		mgoGzip = level
	}
...
func compressInPlace(file string, level int) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
